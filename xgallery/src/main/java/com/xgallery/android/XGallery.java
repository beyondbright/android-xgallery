...
public class XGallery extends FrameLayout implements View.OnTouchListener {
  ...
  private void init(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    int itemMargin = a.getDimensionPixelOffset(R.styleable.XGallery_xGallery_itemMargin, 0);
    a.recycle();
  ...
  private void performClickItem(float x) {
    ...
    int vpWidthExtra = viewPagerMargin + BottomScalePageTransformer.DISTANCE;
    int space = (int) (Math.abs(centerDistance) + mViewPagerWidth / 2.f + vpWidthExtra) / (mViewPagerWidth + vpWidthExtra);
    int position = mViewPager.getCurrentItem() + (isRightSide ? space : -space);
    Log.d("performClickItem", getWidth() + ", " + x + ", " + centerDistance + ", " + isRightSide + ", " + space + ", " + position);
